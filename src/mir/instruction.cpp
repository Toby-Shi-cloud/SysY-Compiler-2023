...
static std::vector<Value *> merge(Value *ptr, std::vector<Value *>::const_iterator cbegin,
                                  std::vector<Value *>::const_iterator cend) {
    std::vector args = {ptr};
...
static std::vector<Value *> flatten(const std::vector<std::pair<T1, T2>> &vec) {
    ...
    for (auto &&[x, y] : vec) ret.push_back(x), ret.push_back(y);
    return ret;
}

std::ostream &Instruction::ret::output(std::ostream &os) const {
    if (auto value = getReturnValue()) return os << "ret " << value;
...
std::ostream &Instruction::br::output(std::ostream &os) const {
    if (hasCondition()) {
        ...
        return os << "br " << getCondition() << ", label " << getIfTrue()->name << ", label " << getIfFalse()->name;
    }
    return os << "br label " << getTarget()->name;
}

std::ostream &Instruction::fneg::output(std::ostream &os) const { return os << name << " = fneg " << getOperand(); }
...
Instruction::getelementptr::getelementptr(pType type, Value *ptr, const std::vector<Value *> &idxs)
    : Instruction(type, GETELEMENTPTR, merge(ptr, idxs.begin() + ptr->type->isPointerTy(), idxs.end())),
      indexTy(getIndexTy(ptr->type)) {}

std::ostream &Instruction::getelementptr::output(std::ostream &os) const {
    os << name << " = getelementptr " << indexTy << ", ptr " << getPointerOperand()->name;
...
std::ostream &Instruction::call::output(std::ostream &os) const {
    if (getFunction()->retType != Type::getVoidType()) os << name << " = ";
...
std::ostream &Instruction::memset::output(std::ostream &os) const {
    return os << "call void @llvm.memset.p0.i32(ptr " << getBase()->name << ", i8 " << val << ", i32 " << size
              << ", i1 false)";
}
}  // namespace mir
