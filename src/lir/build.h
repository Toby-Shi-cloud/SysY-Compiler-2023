...
struct FuncArg {
    ...
    void frame_shrink() { frame_size = std::max(frame_size, (size_t)-frame_cur); }
};
...
inline auto get_node_value(mir::Value *value, CallArg &arg) {
    if (auto val = arg.map[value]) return val;
...
inline void build_phi_node(mir::Instruction::phi *phi, CallArg &arg) { TODO("build_phi_node"); }

inline void build_select_node(mir::Instruction::select *select, CallArg &arg) { TODO("build_select_node"); }
...
#define build_inline(TY, func, ty) \
    case Instruction::TY: func<node::ty>((Instruction::ty *)inst, arg); break
#define build_exact(TY, ty) \
    case Instruction::TY: build_##ty##_node((Instruction::ty *)inst, arg); break
#define build_external(TY, ty) \
    case Instruction::TY: std::invoke(callback, (Instruction::ty *)inst, arg); break
...
#ifdef _DEBUG_
        if (auto it = arg.map.find(inst); it != arg.map.end()) it->second->node->set_original(inst->name);
#endif
    }
...
auto build_dag(mir::Function *func, Callback &&callback, Func &&...fs) {
    return build_dag(func, overloaded{std::forward<Callback>(callback), std::forward<Func>(fs)...});
}
