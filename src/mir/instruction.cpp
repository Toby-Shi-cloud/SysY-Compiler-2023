...
std::ostream &Instruction::br::output(std::ostream &os) const {
    if (hasCondition()) {
        ...
        return os << "br " << getCondition() << ", label " << getIfTrue()->name << ", label "
                  << getIfFalse()->name;
    }
...
std::ostream &Instruction::select::output(std::ostream &os) const {
    return os << name << " = select " << getCondition() << ", " << getTrueValue() << ", "
              << getFalseValue();
}
...
std::ostream &Instruction::memset::output(std::ostream &os) const {
    return os << "call void @llvm.memset.p0.i32(ptr " << getBase()->name << ", i8 " << val
              << ", i32 " << size << ", i1 false)";
}
}  // namespace mir
