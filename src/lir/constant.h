...
#define FOLD(ty, op, tout, tin) template<> inline node::Constant::value_t \
...
#undef FOLD
    ...
    constant_folding_impl<node::frem>(node::Constant::value_t lhs, node::Constant::value_t rhs) {
        return std::fmod((float) lhs, (float) rhs);
    }

#define FOLD(ty) template<> inline node::Constant::value_t constant_folding_impl<node::ty>(node::Constant::value_t val)
...
#undef LL
    ...
    template<> inline std::unique_ptr<DAGNode> constant_folding<node::add>(node::add *node) {
        auto lhs = get_constant(&node->lhs);
        auto rhs = get_constant(&node->rhs);
    ...
    // specific for address computation
    template<> inline std::unique_ptr<DAGNode> constant_folding<node::sub>(node::sub *node) {
        auto lhs = get_constant(&node->lhs);
        auto rhs = get_constant(&node->rhs);
        std::unique_ptr<node::DAGValueNode> n = nullptr;
        if (lhs && rhs) {
...
#define TRY(ty) if (auto val = dynamic_cast<node::ty *>(node)) return constant_folding(val)
...
#undef TRY
    ...
    inline void constant_folding(DAG &dag) {
        ...
        for (auto it = dag.begin(); it != dag.end(); ++it) {
            if (dynamic_cast<node::Constant *>(it->get()))
                queue.push(it);
        }
        while (!queue.empty()) {
        ...
        for (auto it = dag.begin(); it != dag.end();) {
            if (it->get()->in_used()) ++it;
            else it = dag.erase(it);
        }
    }
}
