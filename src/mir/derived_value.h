...
struct calculate_t : std::variant<int, float>, variant_helper<calculate_t> {
    ...
    // NOLINTNEXTLINE
    calculate_t(unsigned value) : calculate_t(static_cast<int>(value)) {}
};
...
struct Function : Value {
    ...
    explicit Function(pType type, const std::string &name)
        : Value(type, "@" + name), retType(type->getFunctionRet()) {}
...
struct Instruction : User {
    ...
    [[nodiscard]] bool isMemoryAccess() const {
        return instrTy == LOAD || instrTy == STORE || instrTy == MEMSET;
    }
    [[nodiscard]] bool isValue() const { return type != Type::getVoidType(); }

    template <typename... Args>
    explicit Instruction(pType type, InstrTy instrTy, Args... args)
...
inline bool isZero(const Value *value) {
    return value == getIntegerLiteral(0) || value == getFloatLiteral(0) ||
