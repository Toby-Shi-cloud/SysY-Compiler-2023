...
struct Instruction::ret : Instruction {
    ...
    [[nodiscard]] Value *getReturnValue() const { return getNumOperands() == 0 ? nullptr : getOperand(0); }
...
struct Instruction::br : Instruction {
    ...
    [[nodiscard]] BasicBlock *getTarget() const { return hasCondition() ? nullptr : getOperand<BasicBlock>(0); }
...
struct Instruction::_binary_instruction : Instruction {
    ...
    [[nodiscard]] std::vector<Value *> getOperands() const override {
        ...
        if constexpr (ty == ADD || ty == MUL || ty == AND || ty == OR || ty == XOR) std::sort(res.begin(), res.end());
        return res;
    }
...
struct Instruction::fneg : Instruction {
    explicit fneg(Value *value) : Instruction(value->type, FNEG, value) { assert(value->type->isFloatTy()); }
...
struct Instruction::icmp : Instruction {
    ...
    explicit icmp(Cond cond, Value *lhs, Value *rhs) : Instruction(Type::getI1Type(), ICMP, lhs, rhs), cond(cond) {
        assert(lhs->type == rhs->type);
    ...
    void interpret(Interpreter &interpreter) const override {
        ...
        interpreter.map[this] = calc((int)interpreter.getValue(getLhs()), (int)interpreter.getValue(getRhs()));
    }
...
struct Instruction::fcmp : Instruction {
    ...
    explicit fcmp(Cond cond, Value *lhs, Value *rhs) : Instruction(Type::getI1Type(), FCMP, lhs, rhs), cond(cond) {
        assert(lhs->type == rhs->type);
    ...
    void interpret(Interpreter &interpreter) const override {
        ...
        interpreter.map[this] = calc((float)interpreter.getValue(getLhs()), (float)interpreter.getValue(getRhs()));
    }
...
struct Instruction::phi : Instruction {
    ...
    void substituteValue(const BasicBlock *which, Value *_new) { substituteOperand(findOperand(which) - 1, _new); }
