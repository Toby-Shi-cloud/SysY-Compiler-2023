...
inline bool Value::isUsed() const {
    ...
    if (auto self = dynamic_cast<const User *>(this); use->users.count(const_cast<User *>(self)))
        return false;
    return true;
}
...
template <typename T>
std::enable_if_t<std::is_base_of_v<Value, T>, std::ostream> &operator<<(std::ostream &os,
                                                                        const T *value) {
    return os << *value;
}
}  // namespace mir
