...
inline std::string to_string(const DAGValue::Type &ty) {
    return std::visit(overloaded{[](const DAGValue::Value &val) -> std::string { return val->to_string(); },
                                 [](const DAGValue::Ch &) -> std::string { return "ch"; },
                                 [](const DAGValue::Glue &) -> std::string { return "glue"; }},
                      ty.as());
}
...
inline bool operator!=(const DAGValue::Type &lhs, const DAGValue::Type &rhs) { return !(lhs == rhs); }
...
struct DAGNode {
    ...
    [[nodiscard]] virtual bool in_used() const {
        auto v = values();
        return std::any_of(v.begin(), v.end(), [](auto &&v) { return !v.users.empty(); });
    }

#ifdef _DEBUG_
    std::string m_node_id;

    void set_original(std::string node_id) { m_node_id = std::move(node_id); }
    ...
    [[nodiscard]] auto to_dot_node() const {
        ...
        if (!my_links.empty()) {
            ...
            for (auto &&l : my_links) {
                ...
                link_labels.emplace_back(dot::label::port{port_name, to_string(l.type)});
                auto v = l.value();
                if (v == nullptr) continue;
                links.emplace_back(dot::link{
                    to_dot_node_name() + ":" + port_name,
                    v->node->to_dot_node_name() + ":o" + std::to_string(v - v->node->values().begin()),
                    std::visit(overloaded{[](const DAGValue::Value &) { return dot::link_type::normal; },
        ...
        if (!values().empty()) {
            ...
            for (auto &&v : values()) {
                value_labels.emplace_back(
                    dot::label::port{"o" + std::to_string(value_labels.size()), to_string(v.type)});
            }
...
#define DAG_NODE_GEN(ty)                                                                  \
    struct ty : DAGNode {                                                                 \
        ...
        ty(DAGValue::Type lhs, DAGValue::Type rhs, DAGValue::Type ret)                    \
            : lhs(this, lhs), rhs(this, rhs), ret(this, ret) {}                           \
...
struct icmp : DAGNode {
    ...
    [[nodiscard]] std::string name() const override {
        return "icmp\\<" + std::string(magic_enum::enum_name(cond)) + "\\>";
    }
};
...
struct fcmp : DAGNode {
    ...
    [[nodiscard]] const_slice<DAGValue> values() const override { return {&ret, 1}; }
    [[nodiscard]] std::string name() const override {
        return "fcmp\\<" + std::string(magic_enum::enum_name(cond)) + "\\>";
    }
};
