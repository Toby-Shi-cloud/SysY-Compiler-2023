...
template <typename T>
std::enable_if_t<std::is_base_of_v<Instruction, T>, inst_node_t> constantFolding(T *inst) {
...
inline bool isPureInst(const Instruction *inst) {
    ...
    if (auto call = dynamic_cast<const Instruction::call *>(inst)) return call->getFunction()->isPure;
    return true;
}

using root_value_t = std::pair<const Value *, std::optional<int>>;
inline root_value_t getRootValue(const Value *value) {
    if (auto load = dynamic_cast<const Instruction::load *>(value)) return getRootValue(load->getPointerOperand());
    if (auto store = dynamic_cast<const Instruction::store *>(value)) return getRootValue(store->getDest());
    if (auto memset = dynamic_cast<const Instruction::memset *>(value)) return getRootValue(memset->getBase());
