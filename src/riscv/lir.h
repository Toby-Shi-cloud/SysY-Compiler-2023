...
namespace riscv::node {
    ...
    struct RetNode : RiscvNode {
        DAGLink ret{mir::Type::getI64Type()}, ch{DAGValue::Ch()}, glue{DAGValue::Glue()};
    ...
    struct CallNode : RiscvNode {
        ...
        CallNode(std::string func, size_t args) : func(std::move(func)) {
            deps.resize(args, DAGLink{mir::Type::getI64Type()});
            deps.emplace_back(DAGValue::Ch());
        }
    ...
    struct Riscv1I1ONode : RiscvNode {
        DAGLink rs;
        DAGValue rd;
        Riscv1I1ONode(DAGValue::Type rs, DAGValue::Type rd) : rs(rs), rd(this, rd) {}
