...
void spiltArray(Manager &mgr, GlobalVar *global) {
    ...
    auto dfs = [&](pType type, Literal *init, auto &&self) -> void {
        ...
        if (type->isNumberTy()) {
            ...
            auto _new_val = new GlobalVar(type, global->name.substr(1) + "." + std::to_string(index), _new_init,
                                          global->isConstLVal());
...
void inlineGlobalVar(Manager &mgr) {
    auto main =
        *std::find_if(mgr.functions.begin(), mgr.functions.end(), std::function<bool(Function *)>(&Function::isMain));
    for (auto &&var : mgr.globalVars) {
        ...
        main->bbs.front()->insert(main->bbs.front()->beginner_end(),
