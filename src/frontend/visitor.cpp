...
std::optional<message> SymbolTable::insert(std::string_view name, const store_type_t &value, const Token &token) {
    if (stack.back().count(name))
        return message{message::ERROR, 'b', token.line, token.column, "redefinition of '" + std::string(name) + "'"};
...
SysYVisitor::visitExps(GrammarIterator begin, GrammarIterator end, value_vector init_value) {
    ...
    value_list list = {};
    std::transform(begin, end, std::back_inserter(indices), [this, &list](const pcGrammarNode &ptr) -> value_type {
...
SysYVisitor::storeInitValue(value_type var, mir::pType type, value_type initVal, value_vector *indices) {
...
SysYVisitor::return_type SysYVisitor::visit<CompUnit>(const GrammarNode &node) {
    for (auto func : mir::Function::getLibrary()) {
        ...
        if (auto idx = name.find_last_of('_'); idx != std::string_view::npos) name = name.substr(idx + 1);
...
SysYVisitor::return_type SysYVisitor::visit<InitVal>(const GrammarNode &node) {
    ...
    if (std::all_of(values.begin(), values.end(),
                    [](auto val) { return val == mir::getIntegerLiteral(0) || val == mir::getFloatLiteral(0.0f); })) {
...
SysYVisitor::return_type SysYVisitor::visit<ForLoopStmt>(const GrammarNode &node) {
    ...
    auto is_semicn = [](const pcGrammarNode &ptr) { return ptr->type == Terminal && ptr->getToken().type == SEMICN; };
...
SysYVisitor::return_type SysYVisitor::visit<AssignStmt>(const GrammarNode &node) {
    ...
    if (variable->isConstLVal()) {
        ...
        message_queue.push_back(message{message::ERROR, 'h', line, column, "cannot assign to const variable"});
        return {nullptr, list};
    }
...
SysYVisitor::return_type SysYVisitor::visit<BreakStmt>(const GrammarNode &node) {
    ...
    if (loop_stack.empty()) {
        ...
        message_queue.push_back(message{message::ERROR, 'm', line, column, "break statement not within a loop"});
        return {nullptr, {}};
    }
...
SysYVisitor::return_type SysYVisitor::visit<ContinueStmt>(const GrammarNode &node) {
    ...
    if (loop_stack.empty()) {
        auto &[ty, raw, line, column] = node.children[0]->getToken();
        message_queue.push_back(message{message::ERROR, 'm', line, column, "continue statement not within a loop"});
        return {nullptr, {}};
    }
...
SysYVisitor::return_type SysYVisitor::visit<ReturnStmt>(const GrammarNode &node) {
    ...
    if (current_function->retType == mir::Type::getVoidType() && node.children.size() == 3) {
        message_queue.push_back(message{message::ERROR, 'f', line, column, "void function should not return a value"});
        return {nullptr, {}};
    }
    if (current_function->retType != mir::Type::getVoidType() && node.children.size() == 2) {
        message_queue.push_back(message{message::ERROR, 'f', line, column, "Non-void function should return a value"});
        return {nullptr, {}};
    }
...
SysYVisitor::return_type SysYVisitor::visit<LVal>(const GrammarNode &node) {
    ...
    if (variable == nullptr) {
        message_queue.push_back(
            message{message::ERROR, 'c', line, column, "undefined symbol '" + std::string(raw) + "'"});
        return {undefined, {}};
    }
...
SysYVisitor::return_type SysYVisitor::visit<UnaryExp>(const GrammarNode &node) {
    ...
    if (node.children[0]->type == Terminal) {
        ...
        if (function == nullptr) {
            message_queue.push_back(
                message{message::ERROR, 'c', line, column, "symbol '" + std::string(raw) + "' is not a function"});
            return {zero_value, {}};
        }
        value_list list = {};
        if (node.children.size() == 3) {
            ...
            if (function->type->getFunctionParamCount() != 0)
                message_queue.push_back(message{message::ERROR, 'd', line, column,
        ...
        if (function->type->getFunctionParamCount() != value.size()) {
            message_queue.push_back(message{
                ...
                std::string("too ") + (function->type->getFunctionParamCount() < value.size() ? "many" : "few") +
                    " arguments for function '" + std::string(raw) + "'"});
        } else {
            for (int i = 0; i < value.size(); i++) {
                if (!value[i]->type->convertableTo(function->type->getFunctionParam(i))) {
                    message_queue.push_back(message{message::ERROR, 'e', line, column,
    ...
    case MINU:
        if (value->type->isIntegerTy())
            value = new Instruction::sub(zero_value, convert_to(value, mir::Type::getI32Type(), false, list));
        else
