...
token_opt Lexer::next_token_impl() {
    while (next_token_skip_comment()) continue;
...
const static auto operators = []() {
    ...
    std::sort(arr.begin(), arr.end(),
              [](auto x, auto y) { return x.first.length() > y.first.length(); });
    return arr;
}();

token_opt Lexer::next_token_try_operator() {
    auto it = std::find_if(operators.cbegin(), operators.cend(), [this](auto v) {
        return v.first == current.substr(0, v.first.length());
    });
...
token_opt Lexer::next_token_try_identifier() {
    ...
    auto start = current;
    auto _column = column();
    while (!current.empty() && (current[0] == '_' || std::isalnum(current[0])))
        current = current.substr(1);
    auto raw = start.substr(0, current.begin() - start.begin());
    if (auto ty = keywords.find(raw); ty != keywords.end())
        return Token{ty->second, raw, _line, _column};
