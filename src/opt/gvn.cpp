...
struct [[maybe_unused]] std::hash<operator_t> {
    ...
    size_t operator()(const operator_t &v) const noexcept { return _hash_0(v.first) ^ _hash_1(v.second); }
};
...
inline bool irrelevant(const root_value_t &x, const root_value_t &y) {
    ...
    if (dynamic_cast<const Argument *>(y.first) && !dynamic_cast<const Instruction::alloca_ *>(x.first)) return false;
...
inline std::optional<Value *> isUseless(const BasicBlock *bb, const Instruction::store *current) {
    ...
    while (it != bb->instructions.begin()) {
        ...
        if (auto load = dynamic_cast<Instruction::load *>(inst))
            if (load->getPointerOperand() == current->getDest() && load == current->getSrc()) return nullptr;
...
inline void globalVariableNumbering(BasicBlock *bb) {
    ...
    auto _isUseless = [&bb](auto &&inst) -> std::optional<Value *> {
        ...
        if (auto store = dynamic_cast<const Instruction::store *>(inst)) return isUseless(bb, store);
        return std::nullopt;
    };
    ...
    auto _last_relevant = [&](auto &&inst) -> Value * {
        ...
        for (auto it = memory_inst.rbegin(); it != memory_inst.rend(); ++it) {
            ...
            if (auto load = dynamic_cast<Instruction::load *>(other))
                if (self && !self->getFunction()->noPostEffect || !irrelevant(getRootValue(load), root)) return load;
            ...
            if (auto call = dynamic_cast<Instruction::call *>(other))
                if (!call->getFunction()->noPostEffect || inst->instrTy == Instruction::STORE) return call;
        }
        return bb;
    };
    ...
    for (auto it = bb->beginner_end(); it != std::prev(bb->instructions.cend());) {
        ...
        if (auto icmp = dynamic_cast<Instruction::icmp *>(inst)) values.push_back(getIntegerLiteral(icmp->cond));
        ...
        for (auto &&v : key.second)
            if (auto t = dynamic_cast<Instruction *>(v); degrees.count(t)) degrees[t]++, edges[inst].push_back(t);
        ++it;
    }
...
void globalVariableNumbering(const Function *func) {
    ...
    std::sort(sorted_bbs.begin(), sorted_bbs.end(), [](auto &&x, auto &&y) { return x->dom_depth < y->dom_depth; });
