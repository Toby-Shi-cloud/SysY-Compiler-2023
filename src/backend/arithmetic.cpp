...
#define CASE_DO(ty) case Instruction::Ty::ty: return do##ty()
...
namespace mips {
    ...
    inline inst_node_t substitute(rInstruction _inst, rRegister _old, rRegister _new) {
        if (_old->defUsers.size() == 1)
            return _old->swapUseTo(_new), _inst->parent->erase(_inst->node);
    ...
    inst_node_t arithmeticFolding(rBinaryMInst self) {
        ...
        if (self->ty != Instruction::Ty::MUL && self->ty != Instruction::Ty::MULTU) return std::next(self->node);
        ...
        if (lo) {
            ...
            block->insert(self->node, std::make_unique<BinaryIInst>(
                              Instruction::Ty::SLL, dst, src1, __builtin_ctz(*imm)));
            lo->swapUseTo(dst);
        }
        if (hi) {
            auto dst = block->parent->parent->newVirRegister();
            block->insert(self->node, std::make_unique<BinaryIInst>(
                              Instruction::Ty::SRL, dst, src1, 32 - __builtin_ctz(*imm)));
            hi->swapUseTo(dst);
        }
        return block->erase(self->node);
    }
