...
struct Manager {
    ...
    ~Manager() {
        ...
        for (auto globalVar : globalVars) delete globalVar;
    ...
    void allocName() const {
        ...
        for (auto globalVar : globalVars)
            if (auto &cnt = names[globalVar->name]; cnt++) globalVar->name += "." + std::to_string(cnt);
        for (auto function : functions) function->allocName();
    }

    template <typename Func>
    void for_each_func(Func &&f) {
        for (auto func : functions) std::invoke(f, func);
    }

    template <typename Func>
    void for_each_glob(Func &&f) {
        for (auto glob : globalVars) std::invoke(f, glob);
    }
    ...
    void output(std::ostream &os) const {
        for (auto globalVar : globalVars) os << globalVar << std::endl;
        os << std::endl;
        ...
        os << std::endl;
        for (auto function : functions) os << function << std::endl;
