...
static inst_node_t arithmeticFolding(Instruction::add *binary) {
    if (binary->getLhs() == getIntegerLiteral(0)) return substitute(binary, binary->getRhs());  // 0 + x = x
    if (binary->getRhs() == getIntegerLiteral(0)) return substitute(binary, binary->getLhs());  // x + 0 = x
    return binary->node;
}

static inst_node_t arithmeticFolding(Instruction::sub *binary) {
    if (binary->getRhs() == getIntegerLiteral(0)) return substitute(binary, binary->getLhs());  // x - 0 = x
    if (binary->getLhs() == binary->getRhs()) return substitute(binary, getIntegerLiteral(0));  // x - x = 0
    return binary->node;
}
...
static inst_node_t arithmeticFolding(Instruction::mul *binary) {
    ...
    if (binary->getRhs() == getIntegerLiteral(0)) return substitute(binary, getIntegerLiteral(0));  // 0 * x = 0
    if (auto rhs = dynamic_cast<IntegerLiteral *>(binary->getRhs()); rhs && __builtin_popcount(rhs->value) == 1) {
        return substitute(binary, new Instruction::shl(binary->getLhs(), getIntegerLiteral(__builtin_ctz(rhs->value))));
        // x * 2^n = x << n
    }
    if (auto rhs = dynamic_cast<IntegerLiteral *>(binary->getRhs()); rhs && __builtin_popcount(-rhs->value) == 1) {
...
constexpr auto arithmeticFoldingDiv = [](auto binary) {
    if (binary->getLhs() == getIntegerLiteral(0)) return substitute(binary, getIntegerLiteral(0));  // 0 / x = 0
    if (binary->getLhs() == binary->getRhs()) return substitute(binary, getIntegerLiteral(1));      // x / x = 1
    if (binary->getRhs() == getIntegerLiteral(1)) return substitute(binary, binary->getLhs());      // x / 1 = x
    return binary->node;
};
...
constexpr auto arithmeticFoldingRem = [](auto binary) {
    if (binary->getLhs() == getIntegerLiteral(0)) return substitute(binary, getIntegerLiteral(0));  // 0 % x = 0
    if (binary->getLhs() == binary->getRhs()) return substitute(binary, getIntegerLiteral(0));      // x % x = 0
    if (binary->getRhs() == getIntegerLiteral(1)) return substitute(binary, getIntegerLiteral(0));  // x % 1 = 0
    return binary->node;
};
...
static inst_node_t arithmeticFolding(Instruction::urem *binary) {
    ...
    if (auto rhs = dynamic_cast<IntegerLiteral *>(binary->getRhs()); rhs && __builtin_popcount(rhs->value) == 1) {
        return substitute(binary, new Instruction::and_(binary->getLhs(), getIntegerLiteral(rhs->value - 1)));
        // x % 2^n = x & (2^n - 1)
    }
    return binary->node;
}
...
inst_node_t constantFolding(Instruction::_binary_instruction<ty> *binary) {
    ...
    if constexpr (ty == Instruction::ADD || ty == Instruction::MUL || ty == Instruction::AND || ty == Instruction::OR ||
                  ty == Instruction::XOR) {
        if (lhs && !rhs) {
...
inst_node_t constantFolding(Instruction::load *load) {
    constexpr auto calc = [](Value *ptr, auto &&self) -> Literal *const * {
        if (auto var = dynamic_cast<GlobalVar *>(ptr)) return var->isConstLVal() && var->init ? &var->init : nullptr;
