...
namespace dot {
    ...
    inline std::ostream &operator<<(std::ostream &os, const label &l) {
        return std::visit(overloaded{
            [&os](const std::string &str) -> decltype(auto) {
                return os << str;
            },
            [&os](const std::vector<label> &vec) -> decltype(auto) {
                return os << "{" << str::join(vec, "|") << "}";
            },
            [&os](const label::port &port) -> decltype(auto) {
                return os << "<" << port.port << ">" << port.name;
            }
        }, l.value);
    }
    ...
    inline std::ostream &operator<<(std::ostream &os, const link_type &ty) {
        switch (ty) {
            case link_type::normal:
                return os << "";
            case link_type::dashed:
                return os << "style=dashed; color=blue";
            case link_type::red:
                return os << "color=red";
        }
