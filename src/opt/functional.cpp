...
void functionInline(Function *func) {
    for (auto bb_it = func->bbs.begin(); bb_it != func->bbs.end(); ++bb_it) {
        ...
        for (auto inst_it = bb->instructions.begin(); inst_it != bb->instructions.end(); ++inst_it) {
            ...
            start_bb->splice(start_bb->beginner_end(), callee_start_bb, callee_start_bb->instructions.cbegin(),
            ...
            bb->moveTo(ret_bb, [](auto &&user) { return dynamic_cast<Instruction::phi *>(user) != nullptr; });
            break;
        }
    }
}
...
void connectBlocks(Function *func) {
    ...
    for (auto it = func->bbs.begin(); it != func->bbs.end(); ++it) {
        ...
        assert(suc != bb);
        for (auto it2 = suc->instructions.begin(); it2 != suc->phi_end();) it2 = constantFolding(*it2);
        bb->pop_back();
        bb->splice(bb->instructions.cend(), suc, suc->instructions.cbegin(), suc->instructions.cend());
