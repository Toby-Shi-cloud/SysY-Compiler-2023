...
void div2mul(rFunction function) {
    ...
    auto process_udiv = [&](rBinaryMInst inst, unsigned imm) {
        ...
        if (__builtin_popcount(imm) == 1) {
            if (lo) {
                ...
                block->insert(inst->node,
                              std::make_unique<BinaryIInst>(Instruction::Ty::SRL, dst, inst->src1(),
                                                            __builtin_ctz(imm)));
                lo->swapUseTo(dst);
            }
            if (hi) {
                auto dst = function->newVirRegister();
                block->insert(inst->node, std::make_unique<BinaryIInst>(Instruction::Ty::ANDI, dst,
                                                                        inst->src1(), imm - 1));
                hi->swapUseTo(dst);
            }
        ...
        auto dst = temp;
        block->insert(inst->node, std::make_unique<BinaryIInst>(Instruction::Ty::LI, m_reg,
                                                                static_cast<int>(m)));
        block->insert(inst->node,
                      std::make_unique<BinaryMInst>(Instruction::Ty::MULTU, inst->src1(), m_reg));
        block->insert(inst->node, std::make_unique<MoveInst>(temp, PhyRegister::HI));
        if (l != 0) {
            dst = function->newVirRegister();
            block->insert(inst->node,
                          std::make_unique<BinaryIInst>(Instruction::Ty::SRL, dst, temp, l));
        }
        changed = true;
        ...
        if (hi) {
            ...
            block->insert(inst->node, std::make_unique<BinaryIInst>(Instruction::Ty::LI, imm_reg,
                                                                    static_cast<int>(imm)));
            block->insert(inst->node, std::make_unique<BinaryMInst>(Instruction::Ty::MUL, mul_reg,
                                                                    dst, imm_reg));
            auto hi_dst = function->newVirRegister();
            block->insert(inst->node, std::make_unique<BinaryRInst>(Instruction::Ty::SUBU, hi_dst,
                                                                    inst->src1(), mul_reg));
            hi->swapUseTo(hi_dst);
        }
        return block->erase(inst->node);
    ...
    auto process_sdiv = [&](rBinaryMInst inst, int imm) {
        ...
        src_block->push_back(std::make_unique<BranchInst>(Instruction::Ty::BLTZ, inst->src1(),
                                                          neg_block->label.get()));
        auto merge_dest = function->newVirRegister();

        auto imm_reg_1 = function->newVirRegister();
        src_block->push_back(
            std::make_unique<BinaryIInst>(Instruction::Ty::LI, imm_reg_1, abs(imm)));
        ...
        if (imm < 0) {
            ...
            src_block->push_back(std::make_unique<BinaryRInst>(Instruction::Ty::SUBU, pos_dest,
                                                               PhyRegister::get(0), pos_temp));
        }
        ...
        neg_block->push_back(std::make_unique<BinaryRInst>(Instruction::Ty::SUBU, neg_src,
                                                           PhyRegister::get(0), inst->src1()));
        auto imm_reg_2 = function->newVirRegister();
        neg_block->push_back(
            std::make_unique<BinaryIInst>(Instruction::Ty::LI, imm_reg_2, abs(imm)));
        ...
        if (imm > 0) {
            ...
            neg_block->push_back(std::make_unique<BinaryRInst>(Instruction::Ty::SUBU, neg_dest,
                                                               PhyRegister::get(0), neg_temp));
        }
        ...
        if (hi) {
            auto imm_reg = function->newVirRegister();
            auto mul_reg = function->newVirRegister();
            result_sub->insert(inst->node,
                               std::make_unique<BinaryIInst>(Instruction::Ty::LI, imm_reg, imm));
            result_sub->insert(inst->node, std::make_unique<BinaryMInst>(
                                               Instruction::Ty::MUL, mul_reg, merge_dest, imm_reg));
            auto hi_dst = function->newVirRegister();
            result_sub->insert(inst->node,
                               std::make_unique<BinaryRInst>(Instruction::Ty::SUBU, hi_dst,
                                                             inst->src1(), mul_reg));
            hi->swapUseTo(hi_dst);
        }
    ...
    for (auto block_it = function->begin(); block_it != function->end(); ++block_it) {
        for (auto sub_it = (*block_it)->subBlocks.begin(); sub_it != (*block_it)->subBlocks.end();
             ++sub_it) {
            ...
            for (; inst_it != (*sub_it)->end(); do_after()) {
                ...
                inst_it = divInst->ty == Instruction::Ty::DIVU ? process_udiv(divInst, *imm)
...
void clearDuplicateInst(rFunction function) {
    for (auto &&block : all_sub_blocks(function)) {
        for (auto it = block->begin(); it != block->end();) {
            ...
            if (!loadImm->imm->isDyn() && other_imm && *other_imm == loadImm->imm->value) {
                it = block->erase(it);
            } else {
                ++it;
            }
