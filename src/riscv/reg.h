...
struct Register {
    ...
    constexpr Register(bool fp, int id) : fp(fp), id(id) { assert(0 <= id && id < 32); }
    ...
    [[nodiscard]] constexpr auto name() const { return fp ? f_name[id] : x_name[id]; }

    [[nodiscard]] auto type() const { return fp ? mir::Type::getFloatType() : mir::Type::getI64Type(); }
};
